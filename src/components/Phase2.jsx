...
export default function Phase2({ onBack, onNext }) {
  ...
  const handleSend = () => {
    ...
    // Giả lập gọi API Google AI Studio mất 3 giây
    setTimeout(() => {
      setIsLoading(false);
      onNext({ selectedGroup, userPrompt: prompt });
    }, 3000);
  };
  ...
  return (
    ...
    >
      <div className="flex items-center mb-8">
        <button onClick={() => { playClickSound(); onBack(); }} className="btn btn-ghost btn-circle text-base-content/70 hover:text-primary">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex-1 text-center pr-12">
          <h2 className="text-2xl font-bold text-primary">Phân Tích & Thu Hẹp Ngành Học</h2>
          <p className="text-sm text-base-content/60 mt-1">Hệ thống AI ML.NET đề xuất các nhóm ngành phù hợp nhất</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-8">
              ...
              return (
                <motion.div
                  ...
                  className={`
                    ...
                    ${isSelected 
                      ? `${group.border} shadow-lg ${group.bg}` 
                ...
                >
                  ...
                  <h3 className="font-bold text-lg leading-tight mb-2">{group.name}</h3>
                  <p className="text-xs text-base-content/60">Độ tương thích dựa trên điểm số và Holland test</p>
                </motion.div>
              );
            })}
          </div>
        ...
        {/* Chat Area - Animate in when a group is selected */}
        <AnimatePresence>
          {selectedGroup && (
            <motion.div 
            ...
            >
              ...
              <div className="flex-1 flex flex-col justify-end pt-6">
                <p className="font-medium text-lg flex items-center gap-2 mb-4">
                  <span className="w-8 h-8 rounded-full bg-secondary/10 text-secondary flex items-center justify-center font-bold">2</span>
                  Kể thêm về bạn để AI gợi ý chuyên ngành cụ thể:
                </p>
                
                {/* Chat Bubble / Input Area */}
                <div className="relative group">
                  <div className={`absolute -inset-0.5 bg-gradient-to-r from-primary to-secondary rounded-3xl blur opacity-20 group-hover:opacity-40 transition duration-1000 group-hover:duration-200 ${isLoading ? 'animate-pulse opacity-50' : ''}`}></div>
                  <div className="relative bg-base-100 rounded-3xl shadow-sm border border-base-200 p-2 flex items-end gap-2">
                    <textarea
                      ...
                      placeholder={`Vui lòng mô tả chi tiết sở thích, tính cách hoặc mong muốn của bạn đối với ${selectedGroup.name} để hệ thống gợi ý chuyên ngành cụ thể nhất...`}
                      value={prompt}
                    ...
                    <button 
                      ...
                      `}
                    >
                      {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5 -ml-1 mt-1" />}
                    </button>
                  </div>
                </div>
